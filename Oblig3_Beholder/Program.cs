...
namespace Oblig3_Beholder
{
	...
	public class Stabel<T>: Lenkeliste<T>
	{
		...
		public T TaAv()
		{
			if (_top == null) { throw new UgyldigListeIndeks(-1); }
			T data = _top.data;
			_top = null;
			return data;
		}
	...
	/*
	public class SortertLenkeliste<T> : Lenkeliste<T>, IComparable<SortertLenkeliste<T>> where T : IComparable
	{
		Node _top;
		...
		public int CompareTo([AllowNull] SortertLenkeliste<T> other)
		{
...
public int Compare(object x)
{
	...
	if(int.TryParse(n1.data.ToString(), out int data))
	{
		int dataNext = int.Parse(next.data.ToString());
		if (data > dataNext)
			return 1;
		if (data < dataNext)
			return -1;
	...
	{
		return String.Compare(n1.ToString(), next.ToString());
	}
	}
